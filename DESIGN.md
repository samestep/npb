...
**Shell completions are generated by the binary, not maintained beside it.**
`--completions <shell>` (`clap_complete`) prints a script for `bash`, `elvish`,
`fish`, `powershell`, or `zsh` on stdout, so a completion can never drift from
the parser the way a checked-in script would; the Nix build runs it at install
time (`installShellCompletion` in `flake.nix`) and any other packaging can do the
same. It is `exclusive`, like `--clean` (§4) and for the same reason — both are
standalone actions that review nothing, so a review knob alongside either is a
mistake rather than a combination, and one attribute per flag can't be forgotten
